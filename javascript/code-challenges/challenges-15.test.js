...
return arr.filter(e => /^\b(Mr.|Ms.|Mrs.|Dr.)\s\b/.test(e));
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const detectTicTacToeWin = (board) => {
  if (checkieChecker(board[0][0], board[0][1],board[0][2])) {
    return true;
  }
  if (checkieChecker(board[1][0], board[0][1],board[0][2])) {
    return true;
  }
  if (checkieChecker(board[2][0], board[0][1],board[0][2])) {
    return true;
  }
  if (checkieChecker(board[0][0], board[1][0],board[2][0])) {
    return true;
  }
  if (checkieChecker(board[0][1], board[1][1],board[2][1])) {
    return true;
  }
  if (checkieChecker(board[0][2], board[1][2],board[2][2])) {
    return true;
  }
  if (checkieChecker(board[0][0], board[1][1],board[2][2])) {
    return true;
  }
  if (checkieChecker(board[0][2], board[1][1],board[2][0])) {
    return true;
  }
  return false;
};

let checkieChecker = (first, second, third) => {
  return first === second && first === third && first !== '';
};

// wooof that last one was a doosey!
